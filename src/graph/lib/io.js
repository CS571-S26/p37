...
import { colorForDeg, updateStats, resetView } from './graph.js'
...
export function serializeVault() {
  let out = ''
  S.nodes.filter(n => !n.isGhost).forEach((n, i) => {
    if (i > 0) out += '\n---\n\n'
    out += `## Node: ${n.label}\n`
    const linked = S.edges
      .filter(e => e.s === n.id || e.t === n.id)
      .map(e => {
        const otherId = e.s === n.id ? e.t : e.s
        const o = S.nodes.find(x => x.id === otherId)
...
export function saveVault() {
  ...
  a.download = S.vaultFilename || 'vault.md'
  a.click()
...
export function buildGraph(fileData) {
  ...
  fileData.forEach(f => {
    if (f.customColor) {
      const n = S.nodes.find(x => x.label === f.name)
      if (n) {
        n.color = f.customColor
        n._customColor = true
      }
    }
  })
...
export function loadVaultFile(file) {
  if (!file) return
  S.vaultFilename = file.name
