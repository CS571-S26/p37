...
export function draw() {
  const { ctx, W, H, camX, camY, zoom, nodes, edges, hovered, hoveredEdge, linkMode, linkFirst, searchHighlight, focusNodeId } = S
  if (!ctx) return
  ctx.clearRect(0, 0, W, H)
  ctx.fillStyle = '#0d0a08'
  ctx.fillRect(0, 0, W, H)

  ctx.strokeStyle = 'rgba(255,220,180,0.03)'
  ctx.lineWidth = 1
  ...
  edges.forEach(e => {
    ...
    ctx.moveTo(a.x, a.y)
    ctx.lineTo(b.x, b.y)
    if (hov && S.cutMode)       { ctx.strokeStyle = 'rgba(180,80,60,0.9)';    ctx.lineWidth = 2.5 / zoom }
  ...
  if (linkMode && linkFirst && hovered && hovered.id !== linkFirst.id) {
    ctx.beginPath()
    ctx.moveTo(linkFirst.x, linkFirst.y)
  ...
  if (linkFirst) {
    const n = linkFirst
    ctx.beginPath()
    ctx.arc(n.x, n.y, nodeRadius(n.id) + 5 / zoom, 0, Math.PI * 2)
